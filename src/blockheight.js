...
if (flag==0) {
  httpsreq('Bearer '+localStorage.getItem("sessionkey")+'','v1/block/hash/'+blockparam+'','printresults');
}
else {
  httpsreq('Bearer '+localStorage.getItem("sessionkey")+'','v1/block/height/'+blockparam+'','printresults');
}
...
if (txcache[((tindex-1)*transactionsperpage)+1]==undefined) {
  httpsreq('Bearer '+localStorage.getItem("sessionkey")+'','v1/block/txids/'+currentblockhash+'/?pagenumber='+currentbatchnum+'&pagesize=50','updateleftpaginationarray');
...
if(txcache[(pagearray[pagearray.length-1]*transactionsperpage)+1]==undefined){
  currentbatchnum = Math.ceil(pagearray[0] / fixedarrlength);
 currentbatchnum+=1;
  httpsreq('Bearer '+localStorage.getItem("sessionkey")+'','v1/block/txids/'+currentblockhash+'/?pagenumber='+currentbatchnum+'&pagesize=50','updatepaginationarray');
}
else{
  currentbatchnum = Math.ceil(pagearray[0] / fixedarrlength);
 currentbatchnum+=1;
  var tindex=0;
  for(var t=(pagearray[fixedarrlength-1]+1);t<=(pagearray[pagearrlength-1]+pagearrlength);t++){
    if (t<=numberofpages) {
      pagearray[tindex] = t;
    }
    else {
...
function transactionprinting(){
  ...
  for (var k = txnumber; k < numberoftransactions; k++) {
    txsection.innerHTML+= "<tr class='txrows'><td>"+(txnumber+(printbreaker))+" - <a href='"+trasactionPath+"?transaction="+txcache[k]+"'>"+txcache[k]+"</a></td></tr>";
